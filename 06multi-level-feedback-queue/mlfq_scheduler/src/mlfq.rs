...
impl MLFQ {
    ...
    pub fn add_process(&mut self, process: Process) {
        ...
        let priority = if process.priority >= self.num_levels {
            self.num_levels - 1  // If priority is too high, we assign the lowest priority that we have
        } else {
            process.priority // or else we get its current priority
        };
        self.queues[priority].push(process);  // we then add the process to the queue
    }
    ...
    pub fn execute_process(&mut self, queue_index: usize) {
        ...
        // first we fetch and remove the process at the front of the specified queue
        if let Some(mut process) = self.queues[queue_index].pop() {
            let time_quantum = self.time_quanta[queue_index];
            //then, lets calculate how much of the process can execute within the quantum, using min
            let execution_time = std::cmp::min(time_quantum, process.remaining_time);
    
            // Update the process's execution stats
            ...
            if process.remaining_time > 0 {
                // if not we have to move it to the next lower queue
                let next_queue = std::cmp::min(queue_index + 1, self.num_levels - 1);
                process.priority = next_queue;
                self.queues[next_queue].push(process);
            }
        }
    }
    ...
    pub fn priority_boost(&mut self) {
        ...
        // lets loop through all queues starting from the second one up to the last priority queue.
        for i in 1..self.num_levels {
            //we pop the queue and set the priority to 0
